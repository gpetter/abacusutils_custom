...
# chop off bit of extra after a slab?
def rectify_slabs(pos1, lbox, slab_frac):
    max_x = -lbox/2. + slab_frac * lbox
    inp_x = pos1[0]
    good_idx = np.where(inp_x < max_x)
    return (pos1[0][good_idx], pos1[1][good_idx], pos1[2][good_idx])
...
def calc_xirppi_fast(x1, y1, z1, rpbins, pimax,
    ...
    if frac_slabs < 1:
        ...
        boxsize = (-1, lbox, lbox)
        x1, y1, z1 = rectify_slabs((x1, y1, z1), lbox, frac_slabs)
    ...
    if autocorr == 1:
        results = DDrppi(autocorr, Nthread, pimax, rpbins, x1, y1, z1,
            boxsize = boxsize, periodic = True, max_cells_per_dim = num_cells, verbose = False)
    ...
    RR_counts_new = np.pi * (rpbins[1:] ** 2 - rpbins[:-1] ** 2) * pi_bin_size / (frac_slabs * lbox ** 3) * ND1 * ND2 * 2
...
def calc_multipole_fast(x1, y1, z1, rpbins,
    ...
    if autocorr == 1:
        ...
        results = DDsmu(autocorr, Nthread, rpbins, 1, nbins_mu, x1, y1, z1, X2 = x2, Y2 = y2, Z2 = z2,
            periodic = True, boxsize = boxsize, max_cells_per_dim = num_cells)
        DD_counts = results['npairs']
...
def calc_wp_fast(x1, y1, z1, rpbins, pimax,
    lbox, frac_slabs, Nthread, num_cells = 30, x2 = None, y2 = None, z2 = None, quiet=True):  # all r assumed to be in h-1 mpc units.
    if not isinstance(pimax, int):
        raise ValueError("pimax needs to be an integer")
    lbox = np.float32(lbox)
    boxsize = lbox
    ...
    if autocorr == 1:
        ...
        results = DDrppi(autocorr, Nthread, pimax, rpbins, x1, y1, z1,
            boxsize = boxsize, periodic = True, max_cells_per_dim = num_cells)
        DD_counts = results['npairs']
    else:
        if not quiet:
    ...
    RR_counts = np.pi*(rpbins[1:]**2 - rpbins[:-1]**2) / (frac_slabs * lbox ** 3) * ND1 * ND2 * 2
